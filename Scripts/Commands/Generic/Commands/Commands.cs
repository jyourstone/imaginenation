...
namespace Server.Commands.Generic
{
    	...
    	public class AddToBankCommand : BaseCommand
	{
		public AddToBankCommand()
		{
			AccessLevel = AccessLevel.GameMaster;
			Supports = CommandSupport.All;
			Commands = new string[]{ "AddToBank", "AddToCont" };
			ObjectTypes = ObjectTypes.Both;
			ListOptimized = true;
			Usage = "AddToBank <name> [params] [set {<propertyName> <value> ...}]";
			Description = "Adds an item by name to the bank of a targeted player. Optional constructor parameters. Optional set property list.";
		}
		...
		public override void ExecuteList( CommandEventArgs e, ArrayList list )
		{
			...
			for ( int i = 0; i < list.Count; ++i )
			{
				...
				if ( obj is PlayerMobile )
					cont = ((PlayerMobile)obj).BankBox;

				if ( cont != null )
					banks.Add( cont );
				else
					LogFailure( "That is not a playermobile." );
			}
	...
	public class TeleCommand : BaseCommand
	{
		...
		public override void Execute( CommandEventArgs e, object obj )
		{
			...
			if ( !from.Hidden )
			{
				Effects.SendLocationParticles( EffectItem.Create( fromLoc, from.Map, TimeSpan.FromSeconds( 1.5 ) ), 0x3709, 10, 20, 2023 );
				Effects.SendLocationParticles( EffectItem.Create(   toLoc, from.Map, TimeSpan.FromSeconds( 1.5 ) ), 0x3709, 10, 20, 2023 );
				from.PlaySound( 499 );

				if ( !from.Mounted )
					from.Animate( 263, 7, 1, true, false, 0 );
				else
					from.Animate( 27, 7, 1, true, false, 0 );
			}
		}
	}
	...
	public class DeleteCommand : BaseCommand
	{
		...
		public override void ExecuteList( CommandEventArgs e, ArrayList list )
		{
			if ( list.Count > 1 )
			{
				e.Mobile.SendGump( new WarningGump( 1060637, 30720, String.Format( "You are about to delete {0} objects. This cannot be undone without a full server revert.<br><br>Continue?", list.Count ), 0xFFC000, 420, 280, OnConfirmCallback, new object[]{ e, list } ) );
		...
		public override void Execute( CommandEventArgs e, object obj )
		{
			...
			if ( m_Value )
			{
				...
				else
				{
					...
					mob.BoltEffect( 0 ); //Iza - Sphere lightning effect on [kill
					mob.Kill();
