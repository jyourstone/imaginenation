...
namespace Server.Mobiles
{
    ...
    public class XmlSpawner : Item, ISpawner
    {
        ...
        public static AccessLevel DiskAccessLevel = AccessLevel.Seer; // minimum access level required by commands that can access the disk such as XmlLoad, XmlSave, and the Save function of XmlEdit **Nasir - Changed to seer
        ...
        private TimerPriority m_BasePriority = TimerPriority.FiveSeconds;
        private TimerPriority m_InactivePriority = TimerPriority.OneMinute;
        ...
        public TimerPriority InactivePriority { get { return m_InactivePriority; } set { m_InactivePriority = value; } }
...
public static void _TraceEnd(int index)
{
	   if(index < MaxTraces){
        ...
        private bool ValidPlayerTrig(Mobile m)
        {
            ...
            return ((m.Player || m_AllowNPCTriggering) && (m.AccessLevel <= TriggerAccessLevel) && (m.AccessLevel < AccessLevel.Administrator) && ((!m.Body.IsGhost && !m_AllowGhostTriggering) || (m.Body.IsGhost && m_AllowGhostTriggering)));
        }
        ...
        {
            XmlSpawner.LoadSettings(AssignSettings, "XmlSpawner");
            ...
            CommandSystem.Register("XmlSpawnerRespawnAll", AccessLevel.GameMaster, RespawnAll_OnCommand);
            ...
            foreach (SpawnObject so in m_SpawnObjects)
            {
                for (int x = 0; x < so.SpawnedObjects.Count; x++)
                {
                    ...
                    if (o is Item)
                    {
                        ...
                        if (DespawnTime.TotalHours > 0 && !item.Deleted && (item.LastMoved < DateTime.Now - DespawnTime) && (item.Parent == Parent)
        ...
        private class SpawnerTimer : Timer
        {
                ...
                : base(delay)
            {
                ...
                if (spawner.IsInactivated)
                {
                    Priority = spawner.InactivePriority;
                }
                else
                {
            ...
            protected override void OnTick()
            {
                //Delay the spawning if the world is saving
                if (World.Saving)
                    new SpawnerTimer(m_Spawner, TimeSpan.FromMinutes(1));
                else
                {
                    if (m_Spawner != null && !m_Spawner.Deleted)
                        m_Spawner.OnTick();
