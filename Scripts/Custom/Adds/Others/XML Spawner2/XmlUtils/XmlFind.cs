			...
			if (!criteria.Dosearchattach)
			{
				...
				if (itemarray != null)
				{
					foreach (Item i in itemarray)
					{
                        ...
                        if (i.RootParentEntity != null && i.RootParentEntity is PlayerMobile)
                        {
                            PlayerMobile pm = (PlayerMobile)i.RootParentEntity;

                            //Taran: Do not display items that are on staff members with higher privs
                            if (pm.AccessLevel > criteria.Accesslev)
                                continue;
                        }
			...
			if (!criteria.Dosearcherr && !criteria.Dosearchattach)
			{
				...
				if (mobilearray != null)
				{
					foreach (Mobile i in mobilearray)
					{
						...
						if (i == null || i.Deleted || i.AccessLevel > criteria.Accesslev) continue;

                        //Taran: Do not display mounts that are ridden by someone with higher priv
                        if (i is BaseMount)
                        {
                            BaseMount bm = (BaseMount) i;
                            if (bm.m_Rider != null)
                            {
                                if (bm.m_Rider is PlayerMobile && bm.m_Rider.AccessLevel > criteria.Accesslev)
                                    continue;
                            }
                        }
				...
				case 3: // Search
					{
                        ...
                        m_SearchCriteria.Accesslev = state.Mobile.AccessLevel;
