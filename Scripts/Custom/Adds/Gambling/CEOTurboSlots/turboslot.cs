...
namespace Server.Items
{
	public class TurboSlot : Item
	{
		...
		private readonly string[] m_ProfReel = { null, null, null };
		...
		private readonly int[,] m_CurrentDist = new int[3, 8];

		// Odds Table and Jackpot events
		private int m_MaxRoll = 100;
		private readonly int[,] m_ReelTable = new int[3, 100];
		...
		private readonly int[] m_jackpotStats = new int[11];
		...
		public bool Active
		{
			...
			set
			{
				if (!m_Active && value)
				{
					...
					PublicOverheadMessage(0, Hue, false, "CEOTurboSlot online!");
				}
				else if (m_Active && !value)
				{
					m_OrigHue = Hue;
					Hue = 1001;
					PublicOverheadMessage(0, Hue, false, "CEOTurboSlot offline.");
				}
				m_Active = value;
		...
		public bool ProgIsMaster
		{
			...
			set
			{
				...
				{
					PublicOverheadMessage(0, Hue, false, "This machine can not be a Slot Master while linked to another Slot Master.");
					return;
				}
			...
			set
			{
				...
				if (value != null && value is TurboSlot && this != value && !value.Deleted)
				{
					if (((TurboSlot)value).ProgIsMaster)
					{
						...
						PublicOverheadMessage(0, Hue, false, "The machine you selected is not a Slot Master.");
				}
				else if (value == null)
				{
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			{
				from.SendMessage("You must be carrying your Golden Coin Casino Membership Card to play this slot.");
				return;
			}
		...
		public string[] GetJackpotPayoutStr(int index, out decimal payout)
		{
			...
			if (m_jackpotmultiplier[index] >= 0 && (index <= 10 && index > -1))
			{
				...
				else
				{
					if (index == 9 || index == 10)
					{
						payout = ((int)(m_Cost * (m_jackpotmultiplier[index] / 100.00)));
		...
		public void JackpotHit(Mobile from, int jackpot, TurboSlot slot)
		{
			...
			//InvalidateProperties();		
			Effects.PlaySound(new Point3D(X, Y, Z), Map, 1035);
			from.FixedParticles(0x375A, 9, 20, 5027, EffectLayer.Waist);
			Effects.SendLocationEffect(new Point3D(X, Y, Z), Map, 0x375A, 15, Hue, 0);
			...
			PublicOverheadMessage(0, Hue, false, text);
			if (m_SlotSlaves != null)
			{
				...
				foreach (TurboSlot s in m_SlotSlaves)   // Announce the jackpot, but not to every machine to avoid a messy screen
				{
					...
					if (announce)
					{
						s.PublicOverheadMessage(0, Hue, false, text);
						Effects.PlaySound(new Point3D(s.X, s.Y, s.Z), s.Map, 1035);
						announcedSlots.Add(s);
					}
					Effects.SendLocationEffect(new Point3D(s.X, s.Y, s.Z), s.Map, 0x375A, 15, Hue, 0);
					loc = s.Location;
				}
			}
		}
		...
		public void DoSpin(Mobile from)
		{
			...
			if ((m_ProgressiveMaster != null && !m_ProgressiveMaster.Deleted && ((TurboSlot)m_ProgressiveMaster).ProgIsMaster) || m_isProgMaster)
			{
				...
				{
					float amount = (m_Cost * (float)(m_ProgressivePercent / 100.00));
					if (amount < 1)
						amount = 1;
				...
				if (r1index == m_Symbols[8] && m_BonusRound != BonusRoundType.None && !m_Profile)
				{
					switch (m_BonusRound)
					{
						case BonusRoundType.MinerMadness:
							{
								m_jackpotStats[5]++;
								PublicOverheadMessage(0, Hue, false, "-Bonus Round!-");
						...
						case 1:
							{
								if (m_jackpotmultiplier[9] == -1)
									...
									DoSpinWin(from, 9, (int)(m_Cost * (m_jackpotmultiplier[9] / 100.00)));
								break;
							}
						...
						case 2:
							{
								if (m_jackpotmultiplier[10] == -1)
									...
									DoSpinWin(from, 10, (int)(m_Cost * (m_jackpotmultiplier[10] / 100.00)));
								break;
							}
		...
		private void DoSpinWin(Mobile from, int r1index, int winnings, int mobileSound, int jackpotSound, string slotText, int JackpotEffect)
		{
			...
			if (jackpotSound != -1)
				Effects.PlaySound(new Point3D(X, Y, Z), Map, jackpotSound);
			if (slotText != null)
				PublicOverheadMessage(0, Hue, false, slotText);
			if (JackpotEffect != 0)
		...
		private void GiveItem(Mobile to, Item i)
		{
			...
			if (pack != null)
			{
				...
				if (i is PowerScroll || i is StatCapScroll)
				{
					...
					to.SendMessage(Hue, "You have received a model pirate ship!");// You have received a scroll of power!
				else
					to.SendMessage(Hue, text);
			}
		}
		...
		{
			PublicOverheadMessage(0, Hue, false, "Share the wealth!");
		...
		private void FlameStrike()
		{
			try
			{
				Effects.SendLocationEffect(new Point3D(X, Y, Z + 1), Map, 0x3709, 15, Hue - 1, 0);
		...
		private void Sparkle(int JackpotEffect)
		{
			try
			{
				if (JackpotEffect == -2)
				{
					Effects.SendLocationEffect(new Point3D(X, Y, Z + 1), Map, 0x375A, 15, Hue - 1, 0);
		...
		private void SlotOffline(int error)
		{
			m_ErrorCode = error;
			Active = false;
			if (m_InUseBy != null && m_InUseBy.Map != Map.Internal)
			{
				try
				{
					m_InUseBy.CloseGump(typeof(NewMinerBonusGump));
					m_InUseBy.CloseGump(typeof(TurboSlotPayTableGump));
					Effects.PlaySound(new Point3D(X, Y, Z), Map, 484);
				}
				catch { }
			}
		}
		...
		private float CalcOdds(int[,] distTable)
		{
			...
			if (m_AnyBars)
			{
				...
				percentage = (((tbars[0] * tbars[1] * tbars[2]) / rolls));
				if (m_Rewards != JackpotRewardType.None)
				...
				if (payout == -2)
					payout = 0;
				payoutpercentage = ((percentage - anybarminus) * payout);
				odds = odds + payoutpercentage;
#if PROFILE
				if (m_Profile & !m_ProfileAll)
			...
			if (m_ScatterPay != ScatterType.None)
			{
				...
				payout = payout / 100;
				payoutpercentage = ((percentage - scatterjackpotpercent) * (payout));
				odds = odds + payoutpercentage;
#if PROFILE
				if (m_Profile && !m_ProfileAll)
				{
				...
				payout = payout / 100;
				payoutpercentage = ((percentage - (scatterjackpotpercent + scat2percent)) * payout);
				odds = odds + payoutpercentage;
#if PROFILE
				if (m_Profile && !m_ProfileAll)
				{
		...
		{
			private readonly TurboSlot m_slot;
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			{
				if (m_CardClubOnly)
					list.Add(1060658, "Status\tAvailable (Members only Only)");
				else
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			writer.Write(m_SlotSlaves.Count);
				...
				case 0: // Release 0
					{
						...
						m_RandomMin = reader.ReadTimeSpan();
						m_RandomMax = reader.ReadTimeSpan();
	...
	public class CasinoToken : Item   // Free plays on Casino games that are aware of Tokens
	{
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			if (Name == null)
				...
				list.Add(Name);
			list.Add(1060584, Amount.ToString()); // uses remaining:
		}
