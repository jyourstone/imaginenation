...
namespace Server.Gumps
{
    public class OnlineGump : Gump
    {
        ...
        {
            CommandSystem.Register("On", AccessLevel.Player, On_OnCommand);
        }
        ...
        {
            m_Mobile.SendGump(new OnlineGump(m_Mobile, m_Filter));
        }
        ...
        public OnlineGump(Mobile owner, string filter)
            : this(owner, BuildList(owner, filter), 0)
        {
            owner.CloseGump(typeof(OnlineGump));
        }

        public OnlineGump(Mobile owner, List<Mobile> list, int page)
        ...
        public static List<Mobile> BuildList(Mobile owner, string filter)
        {
            ...
            for (int i = 0; i < states.Count; ++i)
            {
                ...
                if (m != null && (!m.Hidden || m.AccessLevel <= owner.AccessLevel || (m is PlayerMobile && ((PlayerMobile)m).VisibilityList.Contains(owner))))
                {
        ...
        public void Initialize(int page)
        {
            ...
            for (int i = 0, index = page * EntryCount; i < EntryCount && index < m_Mobiles.Count; ++i, ++index)
            {
                ...
                AddLabelCropped(x + TextOffsetX, y, EntryWidth - TextOffsetX, EntryHeight, GetHueFor(m_Owner, m), m.Deleted ? "(deleted)" : m.Name);
        ...
        private static int GetHueFor(Mobile from, Mobile m)
        {
            switch (m.AccessLevel)
            {
                case AccessLevel.Owner:
                case AccessLevel.Administrator:
                case AccessLevel.Developer: return 0x516;
                ...
                default:
                    {
                        ...
                        {
                            return Notoriety.GetHue(Notoriety.Compute(from, m));
                        }
                    }
            }
        }
                ...
                default:
                    {
                        ...
                        if (index >= 0 && index < m_Mobiles.Count)
                        {
                            ...
                            else if (Message.CanMessage(from, m))
                            {
                                new Knives.Chat3.SendMessageGump(from, m, "", null, MsgType.Normal);
                            }
                            else
                            {
                                from.SendMessage("You cannot send a message to that person.");
                                from.SendGump(new OnlineGump(from, m_Mobiles, m_Page));
                            }
                        }
