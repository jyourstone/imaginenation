	...
	public class RunebookGump : Gump
	{
		...
		private class InternalPrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
                ...
                if (!m_Book.Dictionary.ContainsKey(from) || (m_Book.Dictionary[from] != from.Location && !m_Book.IsChildOf(from.Backpack)))
                {
                    from.SendAsciiMessage("You cannot do this since you moved after using the runebook");
                    return;
                }
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( m_Book.Deleted || !DesignContext.Check( from ) )
				return;

            int buttonID = info.ButtonID;

            if (buttonID > 0 && (!m_Book.Dictionary.ContainsKey(from) || (m_Book.Dictionary[from] != from.Location && !m_Book.IsChildOf(from.Backpack))))
            {
                from.SendAsciiMessage("You cannot do this since you moved after using the runebook");
                return;
            }
						...
						case 0: // Use charges
						{
							if ( m_Book.CurCharges <= 0 )
							{
                                if (HasSpell(from, 31))
                                {
                                    ...
                                    m_Book.OnTravel();
                                    new RecallSpell(from, null, e, m_Book).Cast();
                                }
                                else
                                {
                                    from.CloseGump(typeof(RunebookGump));
