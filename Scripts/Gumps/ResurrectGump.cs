...
namespace Server.Gumps
{
    ...
    public class ResurrectGump : Gump
    {
        ...
        private readonly Item m_Shrine;
        private readonly int m_Price;
        private readonly bool m_FromSacrifice;
        private readonly double m_HitsScalar;

        public ResurrectGump(Mobile owner)
            : this(owner, owner, null, ResurrectMessage.Generic, false, 0.0)
        {
        }

        public ResurrectGump(Mobile owner, double hitsScalar)
            : this(owner, owner, null, ResurrectMessage.Generic, false, hitsScalar)
        {
        }

        public ResurrectGump(Mobile owner, bool fromSacrifice)
            : this(owner, owner, null, ResurrectMessage.Generic, fromSacrifice, 0.0)
        {
        }

        public ResurrectGump(Mobile owner, Mobile healer)
            : this(owner, healer, null, ResurrectMessage.Generic, false, 0.0)
        {
        }
        ...
        public ResurrectGump(Mobile owner, Mobile healer, ResurrectMessage msg, bool fromSacrifice)
            : this(owner, healer, null, msg, fromSacrifice, 0.0)
        {
        }

        public ResurrectGump(Mobile owner, Mobile healer, Item shrine, ResurrectMessage msg, bool fromSacrifice, double hitsScalar)
            : base(100, 0)
        {
            m_Healer = healer;
            m_Shrine = shrine;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1 || info.ButtonID == 2)
            {
                ...
                {
                    from.SendAsciiMessage("You can't get resurrected in house regions.");
                    return;
                }
                if (m_Healer != null)
                {
                    if ( !from.InRange(m_Healer, 4) )
                    {
                        from.SendAsciiMessage("You must be closer to your healer!");
                        return;
                    }
                    if ( !from.InLOS(m_Healer) )
                    {
                        from.SendAsciiMessage("You must be in line of sight of your healer");
                        return;
                    }
                }
                if (m_Shrine != null)
                {
                    if (!from.InRange(m_Shrine, 2))
                    {
                        from.SendAsciiMessage("You must be closer to the shrine!");
                        return;
                    }
                    if (!from.InLOS(m_Shrine))
                    {
                        from.SendAsciiMessage("You must be in line of sight of the shrine");
                        return;
                    }
                }
                if (m_Price > 0)
