		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			if ( info.ButtonID == 1 && !m_House.Deleted )
			{
				if ( m_House.IsOwner( m_Mobile ) )
				{
					...
					else if( !Guild.NewGuildSystem && m_House.FindGuildstone() != null )
					{
                        ...
                        if (toGive != null)
                        {
                            ...
                            if (pack.TryDropItem(m_Mobile, toGive, false))
                            {
                                if (toGive is BankCheck)
                                {
                                    ...
                                    worth = (worth/10) * 9;
                                    ((BankCheck)toGive).Worth = worth;
                                    m_Mobile.SendMessage("A bank check worth {0} gold has been added to your backpack",((BankCheck) toGive).Worth.ToString());
                            ...
                            {
                                toGive.Delete();
                                m_Mobile.SendMessage("Your backpack is full.");
