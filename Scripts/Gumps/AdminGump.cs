	...
	public class AdminGump : Gump
	{
		...
		public static int GetHueFor( Mobile m )
		{
			...
			switch ( m.AccessLevel )
			{
			    ...
			    default:
				{
                    if (m.Karma <= Misc.NotorietyHandlers.PLAYER_KARMA_RED || m.Kills >= Misc.NotorietyHandlers.KILLS_FOR_MURDER)
                        return 0x21;

                    if (m.Criminal || m.Karma <= Misc.NotorietyHandlers.PLAYER_KARMA_GREY)
                        return 0x3B1;
			...
			switch ( pageType )
			{
				...
				case AdminGumpPage.Administer_Access:
				{
					...
					if (from.AccessLevel >= AccessLevel.Administrator)
					{
                        ...
                        if (from.AccessLevel > AccessLevel.Developer || from.AccessLevel == AccessLevel.Administrator)
						{
							AddButtonLabeled( 220, 290, GetButtonID( 3, 315 ), "Make Developer" );

                            if (from.AccessLevel >= AccessLevel.Owner || from.AccessLevel == AccessLevel.Administrator)
				...
				case AdminGumpPage.AccountDetails_ChangeAccess:
				{
                    ...
                    if (from.AccessLevel > AccessLevel.Administrator || from.AccessLevel == AccessLevel.Administrator)
					{
				...
				case 5:
				{
					switch ( index )
					{
						...
						case 24:
						{
                            ...
                            if ((newLevel < from.AccessLevel || from.AccessLevel == AccessLevel.Administrator) && from.AccessLevel != AccessLevel.Developer)
							{
