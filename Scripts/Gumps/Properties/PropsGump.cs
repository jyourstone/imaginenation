...
namespace Server.Gumps
{
	public class PropertiesGump : Gump
	{
			...
			switch ( info.ButtonID )
			{
				...
				default:
				{
					...
					if ( index >= 0 && index < m_List.Count )
					{
                        ...
                        else if (IsType(type, typeofList))
                        {
                            ArrayList thelist = new ArrayList((ICollection)prop.GetValue(m_Object, null));
                            from.SendGump(new SetListGump(from, thelist, m_Object, m_Stack, m_Page, m_List));
                        }
		...
		private static bool IsType( Type type, Type check )
		{
			...
			return t == check || t.IsSubclassOf( check );
		}
		...
		private static readonly Type typeofCPA = typeof( CommandPropertyAttribute );
