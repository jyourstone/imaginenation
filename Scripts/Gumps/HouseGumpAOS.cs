		...
		public static void ConvertHouse_Callback( Mobile from, bool okay, object state )
		{
			...
			if ( okay && house.IsOwner( from ) && !house.HasRentedVendors )
			{
				...
				if ( e != null )
				{
                    ...
                    if (house.Price == 0)
                        cost = e.Cost - house.DefaultPrice;
                    else
                        cost = e.Cost - house.Price;
					...
					if ( newHouse != null )
					{
                        ...
                        from.SendAsciiMessage("To reclaim your vendors type 'get vendor' inside your house");
                        from.SendAsciiMessage("To reclaim your vendors type 'get vendor' inside your house");
                        from.SendAsciiMessage("To reclaim your vendors type 'get vendor' inside your house");
						return;
					}
					...
					switch ( index )
					{
						case 0: // Convert Into Customizable House
						{
							if ( isOwner && !isCustomizable )
							{
                                ...
                                {
                                    if (mobile != from && mobile.Player)
                                        count++;
                                }

                                if (count > 0)
                                    from.SendAsciiMessage("You cannot do this while there are players inside your house");

                                else if (from.Hits < from.HitsMax)
                                    from.SendAsciiMessage("You must have full HP to do this");
