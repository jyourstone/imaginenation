	...
	public class PlayerVendor : Mobile
	{
		...
		public int ChargePerDay
		{
			...
			{ 
				...
				{
                    return 500;
                    /* Taran: Changes to vendors always charge 500gp a day no matter what.
                     * long total = 0;
					foreach ( VendorItem vi in m_SellItems.Values )
					{
						total += vi.Price;
		...
		public virtual void Destroy( bool toBackpack )
		{
			 ...
			 *       -> put the items into a backpack. //Taran: Put in bank instead
			...
			if ( list.Count > 0 || HoldGold > 0 ) // No case 1
			{
				...
				else if ( ( toBackpack || House == null || !House.IsAosRules ) && Map != Map.Internal ) // Case 3 - Move to backpack
				{
                    ...
                    if (Owner != null)
                        Owner.BankBox.DropItem(backpack); // Taran: Move to bank instead if owner exists
                    else
		...
		public override bool OnDragDrop( Mobile from, Item item )
		{
            ...
            else if (item.DonationItem)
		    {
                from.SendAsciiMessage("You are not allowed to sell items received through donations or imagine nickels");
		        return false;
		    }
		    else
			{
		...
		private class PayTimer : Timer
		{
			...
			{
                //Taran: Changed to vendors always charge per real day, otherwise they would be too expensive
				//if ( BaseHouse.NewVendorSystem )
			...
			{
				m_Vendor.m_NextPayTime = DateTime.Now + Interval;
			...
			private void SetInfo( Mobile from, int price, string description )
			{
				...
				if ( price < 0 ) // Not for sale
				{
                    ...
                    if (item is Container)
                    {
                        ...
                        setPrice = true;
                    /* Taran: Allow all items to be not-for-sale to better filter the json vendor data
