...
namespace Server.Mobiles
{
	...
	public abstract class BaseAI
	{
		...
		#region Iza
		private readonly string[] m_GreetKeyword = new string[] { "Hello", "Hail", "Greet" };
		private readonly string[] m_PartKeyword = new string[] { "Bye", "Goodbye", "Garewell" };

		private readonly string[] m_PartRespone = new string[] { "I'm sure we will meet again.", "Goodbye!", "Bye." };
		private readonly string[] m_GreetRespone = new string[] { "Hi", "Hi!", "I greet thee.", "Hello", "Hello!", "Hail to thee" };
		#endregion
		...
		public virtual void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			if( from.Alive && m_Mobile.Controlled && from.InRange( m_Mobile, 14 ) )
			{
				if( from == m_Mobile.ControlMaster )
				{
                    ...
                    //if( m_Mobile.CanDrop ) Taran: Disabled drop to prevent easy llama bombing
		...
		public virtual bool Obey()
		{
			...
			switch( m_Mobile.ControlOrder )
			{
				...
				//case OrderType.Drop:  Taran: Disabled drop to prevent easy llama bombing
				//return DoOrderDrop();
		...
		public virtual void OnCurrentOrderChanged()
		{
			...
			switch( m_Mobile.ControlOrder )
			{
				...
				m_Mobile.Combatant = null;
				break;
                /* Taran: Disabled drop to prevent easy llama bombing
                case OrderType.Drop:
                m_Mobile.ControlMaster.RevealingAction();
                m_Mobile.CurrentSpeed = m_Mobile.PassiveSpeed;
                m_Mobile.PlaySound( m_Mobile.GetIdleSound() );
        ...
        */
                ...
                if (canOpenDoors || canDestroyObstacles)
                {
                    ...
                    if (map != null)
                    {
                        ...
                        while (m_Obstacles.Count > 0)
                        {
                            ...
                            {
                                m_Mobile.DebugSay("Ugabooga. I'm so big and tough I can destroy it: {0}",
                                                    item.GetType().Name);
		...
		public virtual bool AcquireFocusMob( int iRange, FightMode acqType, bool bPlayerOnly, bool bFacFriend, bool bFacFoe )
		{
			...
			{
				m_Mobile.DebugSay("I am controlled");
			...
			if( map != null )
			{
				...
				foreach( Mobile m in eable )
				{
					...
					if ( m_Mobile.Summoned && m_Mobile.Controlled && m_Mobile.SummonMaster != null )
					{
