...
locked down container.
		...
		public override bool OnDragDropInto( Mobile from, Item item, Point3D p )
		{
			...
			/*if ( house != null && house.IsLockedDown( this ) )
			{
				if ( item is VendorRentalContract || ( item is Container && ((Container)item).FindItemByType( 

typeof( VendorRentalContract ) ) != null ) )
				{
					from.SendLocalizedMessage( 1062492 ); // You cannot place a rental contract in a 
...
locked down container.
		...
		{
            if (from.AccessLevel > AccessLevel.Player || (from.InRange(GetWorldLocation(), 3) && from.InLOS(this)) || RootParent is PlayerVendor)
				Open( from );
			else
		...
		public override int GetDroppedSound(Item item)
		{
			return 0x57;
		}
    ...
    public class Pouch : TrapableContainer, IDyable
	{
		[Constructable]
