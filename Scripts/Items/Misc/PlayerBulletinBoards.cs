...
namespace Server.Items
{
    ...
    public class PlayerBBEastDeed : Item
    {
        [Constructable]
        public PlayerBBEastDeed()
        {
            ItemID = 5360;
            Name = "Bulletin board deed (east)";
        }

        public PlayerBBEastDeed(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            if (house != null && house.IsOwner(from))
            {
                ...
                PlayerBBEast bbe = new PlayerBBEast();
                bbe.MoveToWorld(from.Location, from.Map);
            }
            else
    ...
    public class PlayerBBSouthDeed : Item
    {
        [Constructable]
        public PlayerBBSouthDeed()
        {
            ItemID = 5360;
            Name = "Bulletin board deed (south)";
        }

        public PlayerBBSouthDeed(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            if (house != null && house.IsOwner(from))
            {
                ...
                PlayerBBSouth bbs = new PlayerBBSouth();
                bbs.MoveToWorld(from.Location, from.Map);
            }
            else
                from.SendAsciiMessage("You must place this in a house you own");
