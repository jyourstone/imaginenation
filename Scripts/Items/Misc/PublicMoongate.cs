...
//Iza - no doubleclick
        ...
        public override bool OnMoveOver( Mobile m )
		{
            if ( m.Player)
            {
                //Small delay to match the graphic it takes to move over.
                double delay; 
                if (m.Mounted)
                {
                    if (m.Direction == Direction.Running)
                        delay = 0;
                    else
                        delay = 80;
                }
                ...
                Timer.DelayCall(TimeSpan.FromMilliseconds(delay), new TimerStateCallback(Teleport), m);
                return true;
            }
        ...
        private static void Teleport(object state)
        {
            Mobile m = ((Mobile) state);
            Point3D currentLocation = m.Location;
            Point3D randomGate;

            do
            {
                randomGate = PMList.Felucca.Entries[Utility.Random(PMList.Felucca.Entries.Length)].Location;
            } while (currentLocation == randomGate);



            BaseCreature.TeleportPets(m, randomGate, Server.Map.Felucca);

            Effects.SendLocationParticles(EffectItem.Create(m.Location, m.Map, EffectItem.DefaultDuration), 0x3728, 10, 10, 2023);
            m.MoveToWorld(randomGate, Server.Map.Felucca);
            Effects.PlaySound(randomGate, Server.Map.Felucca, 0x1FE);
