	...
	public class HouseRegion : BaseRegion
	{
		...
		public override void OnLocationChanged( Mobile m, Point3D oldLocation )
		{
			...
			if ( m_House.InternalizedVendors.Count > 0 && m_House.IsInside( m ) && !m_House.IsInside( oldLocation, 16 ) && m_House.IsOwner( m ) && m.Alive && !m.HasGump( typeof( NoticeGump ) ) )
			{
                ...
                m.SendAsciiMessage("You have vendors to activate, type 'get vendor' inside your house to reclaim them");
			}
		...
		public override bool OnMoveInto( Mobile from, Direction d, Point3D newLocation, Point3D oldLocation )
		{
			...
			else if ( m_House is HouseFoundation )
			{
                ...
                {
                    from.SendAsciiMessage("You cannot enter while the house is being customized");
                    return false;
                }
			}
		...
		public static TimeSpan CombatHeatDelay = TimeSpan.Zero; //changed from 30 to 0. -- should nto be any combat heat on in:X
        ...
        public override void OnSpeech(SpeechEventArgs e)
        {
           ...
           /*else if ( e.HasKeyword( 0x33 ) ) // remove thyself
            ...
            else if (e.HasKeyword(0x28)) // trash barrel
            {
                if (isCoOwner)
                {
                    from.SendAsciiMessage("You place a Trash barrel");
            ...
            else if (e.Speech.Contains("get vendor"))
            {
                BaseHouse house = BaseHouse.FindHouseAt(from);
                if (house != null)
                {
                    if (from.Player && from.Alive && house.InternalizedVendors.Count > 0 && house.IsOwner(from))
                    {
