...
namespace Server.Multis
{
	public abstract class BaseHouse : BaseMulti
	{
		...
		public virtual DecayType DecayType
		{
			get
			{
                if (m_RestrictDecay || !DecayEnabled || DecayPeriod == TimeSpan.Zero || this is TownHouse)
			    ...
			    DateTime firstbuild = DateTime.Now;
                DateTime secondbuild = DateTime.Now;
                ...
                for (int i = 0; i < acct.Length; ++i)
                {
                    ...
                    if (mob != null)
                    {
                        ...
                        {
                            if (houses[j].BuiltOn < firstbuild)
                            {
                                secondbuild = firstbuild;
                                firstbuild = houses[j].BuiltOn;
                            }
                            else if (houses[j].BuiltOn < secondbuild)
                                secondbuild = houses[j].BuiltOn;
                        }
                    }
                }
 
                if (BuiltOn == firstbuild || BuiltOn == secondbuild )
                    return DecayType.Ageless;
		...
		public void MoveAllToCrate()
		{
            ...
            foreach ( Item lockdown in LockDowns )
            {
                if (!lockdown.Deleted)
                {
                    lockdown.IsLockedDown = false;
                    lockdown.IsSecure = false;
                    lockdown.Movable = true;

                    if (lockdown.Parent == null)
                        DropToMovingCrate(lockdown);
                }
            }
		...
		public static void Configure()
		{
            ...
            EventSink.WorldSave += Save; //Taran: Check house decay on world save instead of every minute to reduce lag
		...
		public static bool CheckAccessible( Mobile m, Item item )
		{
			...
			// Commented out so locked down containers in houses aren't secure
		...
		public bool CheckAccessibility( Item item, Mobile from )
		{
			...
			if ( item is Container )
				return true; // changed from "IsCoOwner( from )" so containers in houses can be looted
            //if (item.Stackable)
            //   return true;
            ...
            if ( item is SeedBox )
                return true;
            if ( item is KarmaBong )
                return true;
            if ( item is MovableKarmaBong )
                return true;
            if (item is BasePlayerBB)
                return true;
		...
		public void Release( Mobile m, Item item )
		{
			...
			{
                m.LocalOverheadMessage(MessageType.Regular, 0x3E9, 1042271); // That is not locked down.
            }
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.Write( m_LastRefreshed );
			writer.Write( m_RestrictDecay );

			writer.Write( m_Visits );
		...
		public static bool HasAccountHouse( Mobile m )
		{
            ...
            // 99 houses per account limit
            if(allHouses.Count > 89) return true;
		...
		{
            //Taran: So that private doors won't get the lock changed
            uint oldKeyValue = m_HouseKeyVal;
            
            uint keyValue = CreateKeys( m );

            //Maka
            m_HouseKeyVal = keyValue;
			...
			if ( m_Doors != null )
			{
				for ( int i = 0; i < m_Doors.Count; ++i )
				{
					...
					if ( door != null && (door.KeyValue == oldKeyValue || door.KeyValue <= 0) )
