...
namespace Server.Multis
{
	...
	public class HouseFoundation : BaseHouse
	{
        ...
        public virtual int teleCost
        {
            get
            {
                int teleCost = 0;
                int designTeleCount = DesignState.TeleporterCount();
                int oldTeleCount = teleCount;

                teleCost = (designTeleCount - oldTeleCount) * 25000;
                return teleCost;
            }
        }

        public bool IsTeleporter(Item item)
        {
            return (m_Fixtures != null && (item is HouseTeleporter));
        }
		...
		public void AddFixtures( Mobile from, MultiTileEntry[] list )
		{
			...
			for( int i = 0; i < m_Fixtures.Count; ++i )
			{
                ...
                //Taran: Removed linking of doors
		...
		public static void Initialize()
		{
			...
			CommandSystem.Register( "DesignInsert", AccessLevel.GameMaster, DesignInsert_OnCommand );
		...
		private class DesignInsertTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object obj )
			{
				...
				{
					HouseFoundation house = FindHouseAt( item ) as HouseFoundation;
		...
		public static void Designer_Roof( NetState state, IEntity e, EncodedReader pvSrc )
		{
			...
			if( context != null )// && (Core.SE || from.AccessLevel >= AccessLevel.GameMaster) )  - All roof tiles should be allowed
			{
				// Read data detailing component graphic and location
				int itemID = pvSrc.ReadInt32();
				int x = pvSrc.ReadInt32();
	...
	public class DesignState
	{
        ...
        public static bool IsTeleporter(int itemID)
        {
            itemID &= 0x3FFF;

            if (itemID >= 0x181D && itemID < 0x1828) // Teleports
                return true;
