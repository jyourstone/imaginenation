	...
	public class Party : IParty
	{
		...
		public void SendPublicMessage( Mobile from, string text )
		{
			...
			for ( int i = 0; i < m_Listeners.Count; ++i )
			{
                ...
                if (mob.Party != this)
                    m_Listeners[i].SendMessage(from.SpeechHue,"Party: [{0}]: {1}", from.Name, text);
            }

			SendToStaffMessage( from, "[Party]: {0}", text );
            EventSink.InvokePartyChat(new PartyChatEventArgs(from, text));
		}
		...
		public void SendPrivateMessage( Mobile from, Mobile to, string text )
		{
			...
			for ( int i = 0; i < m_Listeners.Count; ++i )
			{
                ...
                if (mob.Party != this)
                    m_Listeners[i].SendMessage("Party [{0}]->[{1}]: {2}", from.Name, to.Name, text);
            }

            SendToStaffMessage(from, "[Party]->[{0}]: {1}", to.Name, text);
            EventSink.InvokePartyChat(new PartyChatEventArgs(from, to, text));
		}
