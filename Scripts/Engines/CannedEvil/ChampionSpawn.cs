...
namespace Server.Engines.CannedEvil
{
    public class ChampionSpawn : Item
    {
		...
		private Mobile m_CustomChampion;
        ...
        public void Respawn()
        {
            ...
            while (m_Creatures.Count < (70 - (GetSubLevel()*15)))
            {
                Mobile m = Spawn();
                ...
                if (m is BaseCreature)
                {
                    ...
                    if (!m_ConfinedRoaming)
                    {
                        ...
                        bc.RangeHome =
                            (int)
                            (Math.Sqrt(m_SpawnArea.Width*m_SpawnArea.Width + m_SpawnArea.Height*m_SpawnArea.Height)/2);
                    }
                    else
                    {
                        bc.Home = bc.Location;
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            if (m_Active)
            {
                ...
                list.Add(1060660, "Kills\t{0} of {1} ({2:F1}%)", m_Kills, MaxKills, 100.0*((double) m_Kills/MaxKills));
                    // ~1_val~: ~2_val~
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            switch (version)
            {
                ...
                case 0:
                    {
                        if (version < 1)
                            m_SpawnArea = new Rectangle2D(new Point2D(X - 24, Y - 24), new Point2D(X + 24, Y + 24));
                                //Default was 24
