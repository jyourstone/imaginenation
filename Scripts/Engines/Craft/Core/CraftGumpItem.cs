...
namespace Server.Engines.Craft
{
	public class CraftGumpItem : Gump
	{
		...
		public CraftGumpItem( Mobile from, CraftSystem craftSystem, CraftItem craftItem, BaseTool tool ) : base( 40, 40 )
		{
			...
			AddPage( 0 );
			AddBackground( 0, 0, 530, 517, 5054 );    // Solid background
			AddImageTiled( 10, 10, 510, 22, 2624 );   // Menu title, top bar
			AddImageTiled( 10, 37, 150, 198, 2624 );  // Item image bar
			AddImageTiled( 165, 37, 355, 90, 2624 );  // Item name success etc bar
			AddImageTiled( 10, 240, 155, 22, 2624 );  // Skills title bar
			AddImageTiled( 10, 267, 150, 103, 2624 ); // Bar between skills title and material title
			AddImageTiled( 165, 132, 355, 130, 2624 );// Skills bar
			AddImageTiled( 10, 375, 155, 22, 2624 );  // Materials bar
			AddImageTiled( 10, 402, 150, 53, 2624 );  // Bar between materials and other
			AddImageTiled( 165, 267, 355, 130, 2624 );// Ingredients bar
			AddImageTiled( 10, 460, 155, 22, 2624 );  // Other title bar
			AddImageTiled( 165, 402, 355, 80, 2624 ); // Other bar
			AddImageTiled( 10, 487, 510, 22, 2624 );  // Back and make item button bar
			AddAlphaRegion( 10, 10, 510, 499 );       // Transparent background

			AddHtmlLocalized( 170, 40, 150, 20, 1044053, LabelColor, false, false ); // ITEM
			AddHtmlLocalized( 10, 242, 150, 22, 1044054, LabelColor, false, false ); // <CENTER>SKILLS</CENTER>
			AddHtmlLocalized( 10, 377, 150, 22, 1044055, LabelColor, false, false ); // <CENTER>MATERIALS</CENTER>
			AddHtmlLocalized( 10, 462, 150, 22, 1044056, LabelColor, false, false ); // <CENTER>OTHER</CENTER>
			...
			AddButton( 15, 487, 4014, 4016, 0, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 50, 490, 150, 18, 1044150, LabelColor, false, false ); // BACK

			bool needsRecipe = ( craftItem.Recipe != null && from is PlayerMobile && !((PlayerMobile)from).HasRecipe( craftItem.Recipe ) );

			if( needsRecipe )
			{
				AddButton( 270, 487, 4005, 4007, 0, GumpButtonType.Page, 0 );
				AddHtmlLocalized( 305, 490, 150, 18, 1044151, GreyLabelColor, false, false ); // MAKE NOW
			}
			else
			{
				AddButton( 270, 487, 4005, 4007, 1, GumpButtonType.Reply, 0 );
				AddHtmlLocalized( 305, 490, 150, 18, 1044151, LabelColor, false, false ); // MAKE NOW
			}
			...
			if ( craftItem.UseAllRes )
				AddHtmlLocalized( 170, 352 + (m_OtherCount++ * 20), 310, 18, 1048176, LabelColor, false, false ); // Makes as many as possible at once
			...
			if( needsRecipe )
				AddHtmlLocalized( 170, 402 + (m_OtherCount++ * 20), 310, 18, 1073620, RedLabelColor, false, false ); // You have not learned this recipe.
		...
		public void DrawItem()
		{
			...
			{
				AddHtmlLocalized( 170, 402 + (m_OtherCount++ * 20), 310, 18, 1044059, LabelColor, false, false ); // This item may hold its maker's mark
		...
		public void DrawResource()
		{
            ...
            for (int i = 0; i < m_CraftItem.Resources.Count - (cropScroll ? 1 : 0) && i < 6; i++)
            {
				Type type;
				string nameString;
				int nameNumber;
                ...
                //Taran: Only display the name of the color of the ingot when showing basearmor
                if (m_CraftItem.ItemType.BaseType == typeof(BaseArmor))
                {
                    // Resource Mutation
				...
				{
					retainedColor = true;
					AddHtmlLocalized( 170, 402 + (m_OtherCount++ * 20), 310, 18, 1044152, LabelColor, false, false ); // * The item retains the color of this material
					AddLabel( 500, 269 + (i * 20), LabelHue, "*" );
				}

				if ( nameNumber > 0 )
					AddHtmlLocalized( 170, 269 + (i * 20), 310, 18, nameNumber, LabelColor, false, false );
				else
					AddLabel( 170, 269 + (i * 20), LabelHue, nameString );

				AddLabel( 430, 269 + (i * 20), LabelHue, craftResource.Amount.ToString() );
			}
			...
			if ( cropScroll )
				AddHtmlLocalized( 170, 352 + (m_OtherCount++ * 20), 360, 18, 1044379, LabelColor, false, false ); // Inscribing scrolls also requires a blank scroll and mana.
		}
