...
namespace Server.Engines.Craft
{
	public class DefInscription : CraftSystem
	{
		...
		private DefInscription() : base( 2, 5, 1.0 )// base( 1, 1, 3.0 )
		{
		}
		...
		public override int PlayEndingEffect( Mobile from, bool failed, bool lostMaterial, bool toolBroken, int quality, bool makersMark, CraftItem item )
		{
			...
			if ( !typeofSpellScroll.IsAssignableFrom( item.ItemType ) ) //  not a scroll
			{
				...
				else
				{
                    ...
                    else if (makersMark && quality == 2 && from.Skills[SkillName.Inscribe].Base >= 100.0)
			...
			else
			{
                if (failed)
                {
                    if ( ((from.Mana -item.Mana) / 2) < 0)
                        from.Mana = 0;
                    else
                        from.Mana -= (item.Mana / 2);
		...
		public override void InitCraftList() //lowered the mana consumption by 25% on 3-8 level scrolls
		{
			m_Circle = 0;
			m_Mana = 4;
			...
			// Runebook
			index = AddCraft( typeof( Runebook ), 1044294, "Runebook", 95.0, 100.0, typeof( BlankScroll ), 1044377, 20, 1044378 );
			AddRes( index, typeof( RecallScroll ), 1044445, 16, 1044253 );
			AddRes( index, typeof( GateTravelScroll ), 1044446, 8, 1044253 );
            ...
            //KillBook
            index = AddCraft(typeof(KillBook), 1044294, "KillBook", 90.0, 105.0, typeof(BlankScroll), "BlankScroll", 100, 1044378);
            AddRes(index, typeof(Bone), "Bone", 50, "You do not have enough bones to make that");
            AddSkill(index, SkillName.Tinkering, 70.0, 90.0);
		}
	}
}