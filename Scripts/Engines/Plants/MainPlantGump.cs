...
namespace Server.Engines.Plants
{
	public class MainPlantGump : Gump
	{
		...
		private void DrawPlant()
		{
			...
			{
				string message = m_Plant.PlantSystem.GetLocalizedHealth();
				...
				switch ( m_Plant.PlantSystem.Health )
				{
					case PlantHealth.Dying:
					{
						...
						AddHtml( 136, 167, 42, 20, Color(message, 0x00FC00), false, false );
					...
					case PlantHealth.Wilted:
					{
						...
						AddHtml( 132, 167, 42, 20, Color(message, 0x00C207), false, false );
					...
					case PlantHealth.Healthy:
					{
						...
						AddHtml( 129, 167, 42, 20, Color(message, 0x008200), false, false );
					...
					case PlantHealth.Vibrant:
					{
						...
						AddHtml( 129, 167, 42, 20, Color(message, 0x0083E0), false, false );
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
            ...
            if (((info.ButtonID >= 6 && info.ButtonID <= 10) || info.ButtonID == 12) && (!from.InRange(m_Plant.GetWorldLocation(), 3) || !from.InLOS(m_Plant)))
            {
			...
			switch ( info.ButtonID )
			{
				...
				case 6: // Water
				{
                    ...
                    if (!foundUsableWater)
                    {
                        ...
                        from.SendAsciiMessage("Target the container you wish to use to water the " + m_Plant.GetLocalizedPlantStatus().ToString()); // Target the container you wish to use to water the ~1_val~.
                    }
		...
		private void AddPotion( Mobile from, params PotionEffect[] effects )
		{
			...
			else
			{
				...
				if ( m_Plant.ApplyPotion( effects[0], true, out message ) )
				{
					...
					from.Target = new PlantPourTarget( m_Plant );
                    from.SendAsciiMessage("Target the potion you wish to use on the " + m_Plant.GetLocalizedPlantStatus());
					return;
				}
				else
				{
