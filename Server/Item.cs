 ...
 *
 *   $Id: Item.cs 804 2012-01-02 10:24:08Z asayre $
 *
 ***************************************************************************/
...
namespace Server
{
    ...
    public class Item : IEntity, IHued, IComparable<Item>, ISerializable, ISpawnable
    {
		...
		{
            if ((from.Region != null && !from.Region.AllowTrade) || (to.Region != null && !to.Region.AllowTrade))
            {
                from.SendAsciiMessage("Trading is not allowed in this region.");
                to.SendAsciiMessage("Trading is not allowed in this region.");
                return false;
            }

            if (DonationItem)
            {
                from.SendAsciiMessage("You are not allowed to buy, sell or trade items received through donations");
                to.SendAsciiMessage("You are not allowed to buy, sell or trade items received through donations");
                return false;
            }
			return true;
		}
		...
		public virtual void AddNameProperty( ObjectPropertyList list )
		{
			...
			if ( name == null )
			{
				if ( m_Amount <= 1 )
					...
					list.Add( 1050039, "{0}\t#{1}s", m_Amount, LabelNumber ); // ~1_NUMBER~ ~2_ITEMNAME~
			}
			else
			{
				if ( m_Amount <= 1 )
					...
					list.Add( 1050039, "{0}\t{1}s", m_Amount, Name ); // ~1_NUMBER~ ~2_ITEMNAME~
			}
		}
		...
		public virtual DeathMoveResult OnParentDeath( Mobile parent )
		{
			...
			{
				if( parent.Backpack != null && IsChildOf( parent.Backpack ) )
					return DeathMoveResult.RemainEquiped;
				else
					return DeathMoveResult.MoveToBackpack;
			}
			else if ( CheckNewbied() )
			{
				if ( parent.Backpack != null && IsChildOf( parent.Backpack ) )
					return DeathMoveResult.RemainEquiped;
				else
		...
		{
			to.Send(new AsciiMessage(m_Serial, m_ItemID, MessageType.Label, 0x3b2, 3, "", CliLoc.LocToString(number)));
		}

		public void LabelTo( Mobile to, int number, string args )
		{
			to.Send(new AsciiMessage(this.m_Serial, this.m_ItemID, MessageType.Label, 0x3b2, 3, "", string.Format(CliLoc.LocToString(number, args))));
		}

		public void LabelTo( Mobile to, string text )
		{
			to.Send(new AsciiMessage(m_Serial, m_ItemID, MessageType.Label, 906, 3, "", text));
		}
		...
		{
			if (type == AffixType.Prepend)
				to.Send(new AsciiMessage(this.m_Serial, this.m_ItemID, MessageType.Label, 0x3b2, 3, "", string.Format("{0} {1}", affix, CliLoc.LocToString(number))));
			else
				to.Send(new AsciiMessage(this.m_Serial, this.m_ItemID, MessageType.Label, 0x3b2, 3, "", string.Format("{0} {1}", CliLoc.LocToString(number), affix)));
		}

		public void LabelToAffix( Mobile to, int number, AffixType type, string affix, string args )
		{
			if (type == AffixType.Prepend)
				to.Send(new AsciiMessage(m_Serial, m_ItemID, MessageType.Label, 0x3b2, 3, "", string.Format("{0} {1}", affix, string.Format(CliLoc.LocToString(number), args))));
			else
				to.Send(new AsciiMessage(m_Serial, m_ItemID, MessageType.Label, 0x3b2, 3, "", string.Format("{0} {1}", string.Format(CliLoc.LocToString(number, args)), affix)));
		}
			...
			if ( m_Map != map )
			{
				...
				if ( m_Map != null )
				{
					...
					if ( oldLocation.m_X != 0 )
					{
						...
						foreach ( NetState state in eable )
						{
							...
							if ( m.InRange( oldLocation, GetUpdateRange( m ) ) && (!m.HasFilter || m.InLOS(this)))
							{
								if ( remPacket == null )
				...
				if ( m_Map != null )
				{
					...
					foreach ( NetState state in eable )
					{
						...
						if ( m.CanSee( this ) && m.InRange( m_Location, GetUpdateRange( m ) ) && (!m.HasFilter || m.InLOS(this)) )
							SendInfoTo( state );
					}
			...
			else if ( m_Map != null )
			{
				...
				if ( oldLocation.m_X != 0 )
				{
					...
					foreach ( NetState state in eable )
					{
						...
						if ( !m.InRange( location, GetUpdateRange( m ) ) && (!m.HasFilter || m.InLOS(this)))
						{
							if ( removeThis == null )
		...
		{
			if ( dropped.EventItem && !from.IsInEvent && from.AccessLevel < AccessLevel.GameMaster)
			{
				from.SendAsciiMessage( "You can't use event items!" );
				//dropped.Delete();
				return false;
			}
		...
		public virtual void Serialize( GenericWriter writer )
		{
            ...
            //Version 13 DonationItem
            writer.Write(DonationItem);

            //Version 12 EventItemConsume
            writer.Write(EventItemConsume);

            //Version 11 Stealable
            writer.Write(UnStealable);

			//Version 10 INX Changes
			writer.Write( m_EventItem );
		...
		public virtual void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
                ...
                case 13:
			        {
			            DonationItem = reader.ReadBool();
			            goto case 12;
			        }
                //Event Item Consume
                case 12:
			        {
			            EventItemConsume = reader.ReadBool();
			            goto case 11;
			        }
                //Stealable
                case 11:
                    {
                        UnStealable = reader.ReadBool();
                        goto case 10;
                    }
				//INX Changes
				case 10:
				{
					m_EventItem = reader.ReadBool();
					goto case 9;
				}
				case 9:
				case 8:
			...
			if ( map != null && !Deleted )
			{
				...
				if ( (flags & ItemDelta.Update) != 0 )
				{
					...
					foreach ( NetState state in eable ) {
						...
						if ( m.CanSee( this ) && m.InRange( worldLoc, GetUpdateRange( m ) ) && (!m.HasFilter || m.InLOS(this))) {
							if ( m_Parent == null ) {
								SendInfoTo( state, ObjectPropertyList.Enabled );
							} else {
								if ( p == null ) {
		...
		public void PublicOverheadMessage( MessageType type, int hue, int number, string args )
		{
			if ( m_Map != null )
			{
				...
				foreach ( NetState state in eable )
				{
					...
					if ( m.CanSee( this ) && m.InRange( worldLoc, GetUpdateRange( m ) ) && (!m.HasFilter || m.InLOS(state.Mobile)))
					{
						if ( p == null )
							p = Packet.Acquire(new AsciiMessage(m_Serial, m_ItemID, type, hue, 3, "", string.Format(CliLoc.LocToString(number, args))));
        ...
        public void PrivateOverheadMessage(MessageType type, int hue, bool ascii, string text, NetState state)
        {
            if (m_Map != null)
            {
                Packet p = new AsciiMessage(m_Serial, m_ItemID, type, hue, 3, Name, text);
        ...
        public void PrivateOverheadMessage(MessageType type, int hue, int number, string args, NetState state)
        {
            if (m_Map != null)
            {
                Packet p = Packet.Acquire(new AsciiMessage(m_Serial, m_ItemID, type, hue, 3, "", string.Format(CliLoc.LocToString(number, args))));
		...
		public virtual bool DropToMobile( Mobile from, Mobile target, Point3D p )
		{
            ...
            if (target != from && target.Player && from.Region != null && !from.Region.AllowTrade)
            {
                from.SendAsciiMessage("Trading is not allowed in this region.");
                return false;
            }

            if (target != from && DonationItem)
            {
                from.SendAsciiMessage("You are not allowed to buy, sell or trade items received through donations");
                return false;
            }
            ...
            if (m_EventItem)
            {
                if (from.AccessLevel < AccessLevel.GameMaster)
					from.SendAsciiMessage( "Event items can't be dropped!" );
		...
		public virtual bool OnDroppedInto( Mobile from, Container target, Point3D p )
		{
			...
			if ( from.IsInEvent && from.AccessLevel < AccessLevel.GameMaster )
			{
				if ( m_EventItem )
				{
					if ( target.EventItem || target == from.Backpack )
				...
				{
					from.SendAsciiMessage("You can only use event items!");
					return false;
				}
			}
			...
			else if ( m_EventItem && from.AccessLevel < AccessLevel.GameMaster )
			{
				from.SendAsciiMessage( "You can't use event items!" );
				//Delete();
				return false;
		...
		public virtual bool OnDroppedOnto( Mobile from, Item target )
		{
			if ( Deleted || from.Deleted || target.Deleted || from.Map != target.Map || from.Map == null || target.Map == null )
				return false;
			else if ( from.AccessLevel < AccessLevel.GameMaster && !from.InRange( target.GetWorldLocation(), 3 ) )
				return false;
			else if ( !from.CanSee( target ) || !from.InLOS( target ) )
				return false;
		...
		public virtual bool DropToItem( Mobile from, Item target, Point3D p )
		{
			...
			if ( from.AccessLevel < AccessLevel.GameMaster && !from.InRange( target.GetWorldLocation(), 3 ) )
				return false;
		...
		public void SendLocalizedMessageTo( Mobile to, int number )
		{
			...
			to.Send(new AsciiMessage(Serial, ItemID, MessageType.Regular, 0x3B2, 3, "", CliLoc.LocToString(number)));
		}
		...
		public void SendLocalizedMessageTo( Mobile to, int number, string args )
		{
			...
			to.Send(new AsciiMessage(Serial, ItemID, MessageType.Regular, 0x3B2, 3, "", string.Format(CliLoc.LocToString(number, args))));
		}
		...
		public void SendLocalizedMessageTo( Mobile to, int number, AffixType affixType, string affix, string args )
		{
			...
			if ( affixType == AffixType.Prepend )
				to.Send(new AsciiMessage(this.m_Serial, this.m_ItemID, MessageType.Label, 0x3b2, 3, "", string.Format("{0} {1}", affix, string.Format(CliLoc.LocToString(number, args)))));
			else
				to.Send(new AsciiMessage(this.m_Serial, this.m_ItemID, MessageType.Label, 0x3b2, 3, "", string.Format("{0} {1}", string.Format(CliLoc.LocToString(number, args)), affix)));
		}
		...
		{
			from.SendAsciiMessage("Your ghostly hand passes through the object.");
		}
		...
		public virtual void OnSingleClick( Mobile from )
		{
			...
			if ( ns != null )
			{
				...
				{
					if ( m_Amount <= 1 )
						ns.Send(new AsciiMessage(this.m_Serial, this.m_ItemID, MessageType.Label, 0x3b2, 3, "", CliLoc.LocToString(LabelNumber)));
                    else if ( CliLoc.LocToString(LabelNumber).EndsWith("s") )
                        ns.Send(new AsciiMessage(this.m_Serial, this.m_ItemID, MessageType.Label, 0x3b2, 3, "", string.Format("{0} {1}", this.m_Amount, CliLoc.LocToString(LabelNumber))));
                    else
                        ns.Send(new AsciiMessage(this.m_Serial, this.m_ItemID, MessageType.Label, 0x3b2, 3, "", string.Format("{0} {1}s", this.m_Amount, CliLoc.LocToString(LabelNumber))));
				}
				else
				{
					if ( m_Amount <= 1 )
						ns.Send(new AsciiMessage(m_Serial, m_ItemID, MessageType.Label, 0x3b2, 3, "", this.Name));
					else
					{
						string toDisplay;

                        if (this.Name.Contains("crafted by")) //Taran: To-do: Fix so crafted items display s after the item, not after the crafter
                            toDisplay = string.Format("{0} {1}", this.m_Amount, this.Name);
						else if (this.Name[this.Name.Length - 1] != 's')
							toDisplay = string.Format("{0} {1}s", this.m_Amount, this.Name);
						else
							toDisplay = string.Format("{0} {1}", this.m_Amount, this.Name);

						ns.Send(new AsciiMessage(this.m_Serial, this.m_ItemID, MessageType.Label, 0x3b2, 3, "", toDisplay));
					}
				}
			}
		}
