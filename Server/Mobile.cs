 ...
 *
 *   $Id: Mobile.cs 798 2011-12-31 07:50:54Z asayre $
 *
 ***************************************************************************/
...
namespace Server
{
    ...
    {
		#region Custom
		private bool m_InEvent = false;
		private bool m_HasFilter = false;
        ...
        private const int WarmodeCatchCount = 16; // Allow four warmode changes in 0.5 seconds, any more will be delay for two seconds
		...
		private static TimeSpan m_ExpireCriminalDelay = TimeSpan.FromMinutes( 10.0 );
			...
			if( m_Map != null )
			{
				...
				for( int i = 0; i < m_MoveList.Count; ++i )
				{
					...
					if( o is Mobile )
					{
						...
						if( ns != null && Utility.InUpdateRange( m_Location, m.m_Location ) && m.CanSee( this ) && ( !m.HasFilter || InLOS( m ) ) )
						{
                            Packet p = null;
		...
		public void DropHolding()
		{
			...
			if( holding != null )
			{
                ...
                {
                    if (AddToBackpack(holding)) //Taran: Do this if the player isn't overweight (add item to backpack)
                    {
                        Holding = null;
                        holding.ClearBounce();
                    }
                    ...
                    {
                        BankBox.DropItem(holding); //Taran: If the player is overweight, the item drops to bank instead
                        Holding = null;
                        holding.ClearBounce();
                    }
                }
		...
		public virtual void Kill()
		{
			...
			if( m_Map != null )
			{
				...
				{
					if( state != m_NetState && (!state.Mobile.HasFilter || InLOS( state.Mobile ) ) )
					{
						if( animPacket == null )
		...
		private static bool m_NoSpeechLOS = true;
		...
		public virtual void Use( Item item )
		{
			...
			else if( !item.IsAccessibleTo( this ) )
			{
				...
				okay = false;
			else if(AccessLevel == AccessLevel.Player && ( Paralyzed || Frozen ) )
			{
				okay = false;
				SendAsciiMessage("You can't reach that.");
			}
			...
			if( okay )
			{
                if (item.EventItem && !m_InEvent && item != Backpack && AccessLevel < AccessLevel.GameMaster)
				{
					SendAsciiMessage( "You can't use event items!" );
					//item.Delete();
				}
                else if (!item.EventItem && m_InEvent && item != Backpack)
                {
		...
		{
            //NOTE: DO NOT FORGET TO CHANGE THE LIFT OVERRIDE IN PLAYERMOBILE.CS WHEN CHANGES ARE MADE IN HERE
			rejected = true;
			...
			if( from.AccessLevel >= AccessLevel.GameMaster || DateTime.Now >= from.NextActionTime )
			{
				if( from.CheckAlive() )
				{
					...
					else
					{
                        ...
                        else if (item.EventItem && !m_InEvent && AccessLevel < AccessLevel.GameMaster)
						{
							SendAsciiMessage( "You can't use event items!" );
							//item.Delete();
							reject = LRReason.Inspecific;
						}
                        else if (!item.EventItem && m_InEvent && AccessLevel == AccessLevel.Player)
						{
							SendAsciiMessage( "You can only use event items!" );
							reject = LRReason.Inspecific;
						}
		...
		public void AddSpeechItemsFrom( ArrayList list, Container cont )
		{
			...
			if( m_Map != null )
			{
				...
				foreach( object o in eable )
				{
					if( o is Mobile )
					{
						...
						if( heard.CanSee( this ) && (m_NoSpeechLOS || !heard.Player || heard.InLOS( this )) && ( !heard.HasFilter || InLOS( heard ) ) )
						{
		...
		public virtual void SendDamageToAll( int amount )
		{
			...
			foreach( NetState ns in eable )
			{
				if( ns.Mobile.CanSee( this ) && ( !ns.Mobile.HasFilter || InLOS( ns.Mobile ) ) )
				{
					Packet p;
		...
		{
			Heal( amount, this, false );
		}

		public void Heal( int amount, Mobile from )
		{
			Heal( amount, from, false );
		}
		...
		public void Heal( int amount, Mobile from, bool message )
		{
			...
			if( message && amount > 0 && m_NetState != null )
				m_NetState.Send( new AsciiMessage(Serial.MinusOne, -1, MessageType.Label, 0x3B2, 3, "", string.Format("{0}:{1}", string.Format(CliLoc.LocToString(1008158), amount.ToString()))));
		}
		...
		public virtual void Serialize( GenericWriter writer )
		{
			...
			//Version 32 INX Changes
			writer.Write( m_InEvent );
		...
		public void PlaySound( int soundID )
		{
			...
			if( m_Map != null )
			{
				...
				foreach( NetState state in eable )
				{
					if( state.Mobile.CanSee( this ) && (!state.Mobile.HasFilter || InLOS(state.Mobile)))
					{
						if( p == null )
		...
		{
			to.Send( new AsciiMessage(m_Serial, Body, MessageType.Regular, m_SpeechHue, 3, Name, CliLoc.LocToString(number)) );
		}

		public void SayTo( Mobile to, int number, string args )
		{
			to.Send( new AsciiMessage(m_Serial, Body, MessageType.Regular, m_SpeechHue, 3, Name, string.Format(CliLoc.LocToString(number, args))) );
		}
				...
				if( m_Hidden != value )
				{
					...
					if( m_Map != null )
					{
						...
						foreach( NetState state in eable )
						{
							...
							else if( !state.Mobile.HasFilter || InLOS( state.Mobile ) )
							{
                                if (state.StygianAbyss)
					...
					if( ourState != null )
					{
						...
						foreach( object o in eable )
						{
							...
							else if( o != this && o is Mobile )
							{
								...
								if( (isTeleport || !inOldRange) && m.m_NetState != null && m.CanSee( this ) && (!HasFilter || InLOS(m)))
								{
								...
								if( !inOldRange && CanSee( m ) && (!HasFilter || InLOS(m)))
								{
		...
		{
            if (dropped.EventItem && !m_InEvent && AccessLevel < AccessLevel.GameMaster)
            {
                SendAsciiMessage("You can't use event items!");
                return false;
            }
            if (!dropped.EventItem && m_InEvent)
            {
                SendAsciiMessage("You can only use event items!");
                return false;
            }
			...
			else if( from.Player && this.Player && from.Alive && this.Alive && from.InRange( Location, 3 ) )
			{
				NetState ourState = m_NetState;
        ...
        public virtual void ProcessDelta()
		{
			...
			if( m.m_Map != null && (sendRemove || sendIncoming || sendPublicStats || sendHits || sendMoving || sendOPLUpdate || sendHair || sendFacialHair || sendHealthbarPoison || sendHealthbarYellow) )
			{
				...
				foreach( NetState state in eable )
				{
					...
					if( beholder != m && beholder.CanSee( m ) && (!beholder.HasFilter || InLOS(beholder)) )
					{
						if( sendRemove )
							if ( state.StygianAbyss )
                            {
		...
		public void PublicOverheadMessage( MessageType type, int hue, int number, string args, bool noLineOfSight )
		{
			if( m_Map != null )
			{
				...
				foreach( NetState state in eable )
				{
					...
					{
						if( p == null )
							p = Packet.Acquire(new AsciiMessage(m_Serial, Body, type, hue, 3, "", string.Format(CliLoc.LocToString(number, args))));
		...
		public void PrivateOverheadMessage( MessageType type, int hue, int number, string args, NetState state )
		{
			...
			state.Send(new AsciiMessage(m_Serial, Body, type, hue, 3, Name, string.Format(string.Format(CliLoc.LocToString(number, args)))));
		}
		...
		public void LocalOverheadMessage( MessageType type, int hue, int number, string args )
		{
			...
			if( ns != null )
				ns.Send(new AsciiMessage(m_Serial, Body, type, hue, 3, Name, string.Format(string.Format(CliLoc.LocToString(number, args)))));
		}
		...
		public void SendLocalizedMessage( int number )
		{
			...
			if( ns != null )
				SendAsciiMessage(CliLoc.LocToString(number));
		}
		...
		public void SendLocalizedMessage( int number, string args, int hue )
		{
			...
			else
			{
				...
				if( ns != null )
					ns.Send(new AsciiMessage(Serial.MinusOne, -1, MessageType.Regular, hue, 3, "System", string.Format(CliLoc.LocToString(number, args))));
			}
		}
		...
		public void SendLocalizedMessage( int number, bool append, string affix, string args, int hue )
		{
			...
			if( ns != null )
				if( append )
					ns.Send(new AsciiMessage(Serial.MinusOne, -1, MessageType.Regular, hue, 3, "", string.Format("{0} {1}", affix, string.Format(CliLoc.LocToString(number, args)))));
				else
