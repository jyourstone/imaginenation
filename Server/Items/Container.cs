 ...
 *
 *   $Id: Container.cs 544 2010-10-14 06:34:46Z mark $
 *
 ***************************************************************************/
	...
	public class Container : Item
	{
		...
		public virtual bool CheckHold( Mobile m, Item item, bool message, bool checkItems, int plusItems, int plusWeight )
		{
			if ( m.AccessLevel < AccessLevel.GameMaster )
			{
				...
				if ( checkItems && maxItems != 0 && Items.Count >= maxItems )
				{
					if ( message )
		...
		private static int m_GlobalMaxWeight = 0;
		...
		public override void OnSingleClick( Mobile from )
		{
            ...
            if (string.IsNullOrEmpty(Name))
                labelMessage = string.Format("{0} ({1} item{2})", CliLoc.LocToString(LabelNumber), Items.Count, Items.Count == 1 ? "" : "s");
            else
                labelMessage = string.Format("{0} ({1} item{2})", Name, Items.Count, Items.Count == 1 ? "" : "s"); ;
